...
namespace MediaBrowser.WebDashboard.Api
{
    ...
    public class DashboardService : IService, IRequiresRequest
    {
        ...
        {
            const string unavailableMessage = "Kaaisuflix encountered an error and is reloading. Please try again momentarily.";
            ...
            var configPages = pages.Select(p =>
            {
                ...
                {
                    _logger.LogError(ex, "Error getting plugin information from {Plugin}. Please contact a KSF developer.", p.GetType().Name);
                    return null;
                }
            })
                .Where(i => i != null)
        ...
        public async Task<object> Get(GetDashboardPackage request)
        {
            ...
            if (!string.Equals(inputPath, targetPath, StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    _logger.LogError(ex, "Error deleting {Path}. Do you have the correct permissions?", targetPath);
                }
